...
void setPixel(Image& image, int x, int y, char c = 'X') {
...
void drawLine(Image& image, int from_x, int from_y, int to_x, int to_y) {
    setPixel(image, from_x, from_y);   
    setPixel(image, to_y, to_y);   
}
...
void Screen::blit(img::Image& image, int at_x, int at_y) {
    impose(screenImage, image, at_x, at_y);
}
